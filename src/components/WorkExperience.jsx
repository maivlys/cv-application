...
export default function WorkExperience({setWorkExperienceData, workExperienceData}){
  ...
  function handleEditWork(exp) {
    setEditingWork(exp)
    setIsFormOpen(true);
  }

  function handleDeleteWork(exp) {
    const filteredData = workExperienceData.filter((item)=>item.id!==exp.id)
  ...
  return (
    <div className="work-panel">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                <defs>
                  <linearGradient id="gradEdit" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="#29F39B" />
                    <stop offset="100%" stopColor="#02A1F9" />
                  </linearGradient>
                </defs>
      ...
      <ul>
        {workExperienceData.length>0 && workExperienceData.map(item=>
          <li className="work-overview" key={item.id}>
